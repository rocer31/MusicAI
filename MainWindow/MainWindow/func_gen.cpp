...
void realize_config(config &params, unordered_map<string, chord> all_chords, unordered_map<string, int> drum_sounds, fluid_synth_t* synth) {
    ...
    play_chord(all_chords["Imaj7"], all_chords["Imaj7"].get_voicings()[0], all_chords["Imaj7"].get_quality(), all_chords["Imaj7"].get_root() + key, params.q, synth, roots, params.measure, drum_sounds, params.beats);
}
