...
%}
pattern=zeros(33,3,7);

%block 1
pattern(1,1,1)=1; %ID
pattern(1,2,1)=10; %before #1
pattern(1,2,2)=12; %before #2
pattern(1,2,3)=30; %before #3
pattern(1,2,4)=5; %before #4
pattern(1,2,5)=6; %before #5
pattern(1,2,6)=4; %before #6
pattern(1,3,1)=2; %after #1
pattern(1,3,2)=3; %after #2
pattern(1,3,3)=32; %after #3
pattern(1,3,4)=33; %after #4
pattern(1,3,5)=7; %after #5

%block 2
pattern(2,1,1)=2; %ID
pattern(2,2,1)=14; %before #1
pattern(2,2,2)=21; %before #2
pattern(2,2,3)=19; %before #3
pattern(2,2,4)=5; %before #4
pattern(2,2,5)=1; %before #5
pattern(2,3,1)=3; %after #1
pattern(2,3,2)=4; %after #2
pattern(2,3,3)=5; %after #3
pattern(2,3,4)=9; %after #4

%block 3
pattern(3,1,1)=3; %ID
pattern(3,2,1)=1; %before #1
pattern(3,2,2)=2; %before #2
pattern(3,2,3)=13; %before #3
pattern(3,2,4)=15; %before #4
...
pattern(4,2,3)=26; %before #3
pattern(4,2,4)=28; %before #4
pattern(4,3,1)=1; %after #1
pattern(4,3,2)=5; %after #2

%block 5
pattern(5,1,1)=5; %ID
pattern(5,2,1)=16; %before #1
pattern(5,2,2)=18; %before #2
pattern(5,2,3)=3; %before #3
pattern(5,2,4)=4; %before #4
pattern(5,2,5)=2; %before #5
...
%block 7
pattern(7,1,1)=7; %ID
pattern(7,2,1)=31; %before #1
pattern(7,2,2)=24; %before #2
pattern(7,2,3)=14; %before #3
pattern(7,2,4)=5; %before #4
pattern(7,2,5)=1; %before #5
pattern(7,3,1)=9; 
...
%block 9
pattern(9,1,1)=9; %ID
pattern(9,2,1)=24; %before #1
pattern(9,2,2)=3; %before #2
pattern(9,2,3)=5; %before #3
pattern(9,2,4)=2; %before #4
pattern(9,2,5)=32; %before #5
pattern(9,2,6)=8; %before #6
pattern(9,2,7)=33; %before #7

%block 10
pattern(10,1,1)=10; %ID
pattern(10,3,1)=1; %after #1

%block 11
pattern(11,1,1)=11; %ID
pattern(11,3,1)=12; %after #1

%block 12
pattern(12,1,1)=12; %ID
pattern(12,2,1)=11; %before #1
pattern(12,3,1)=1; %after #1

%block 13
pattern(13,1,1)=13; %ID
pattern(13,3,1)=3; %after #1

%block 14
pattern(14,1,1)=14; %ID
pattern(14,3,1)=15; %after #1

%block 15
pattern(15,1,1)=15; %ID
pattern(15,2,1)=14; %before 1
pattern(15,3,1)=2; %after #1

%block 16
pattern(16,1,1)=16; %ID
pattern(16,3,1)=5; %after #1

%block 17
pattern(17,1,1)=17; %ID
pattern(17,3,1)=18; %after #1

%block 18
pattern(18,1,1)=18; %ID
pattern(18,2,1)=17; %before 1
pattern(18,3,1)=5; %after #1

%block 19
pattern(19,1,1)=19; %ID
pattern(19,3,1)=2; %after #1

%block 20
pattern(20,1,1)=20; %ID
pattern(20,3,1)=21; %after #1
pattern(20,3,2)=22; %after #2

%block 21
pattern(21,1,1)=21; %ID
pattern(21,2,1)=20; %before #1
pattern(21,2,2)=22; %before #2
pattern(21,3,1)=2; %after #1

%block 22
pattern(22,1,1)=22; %ID
pattern(22,2,1)=20; %before #1
pattern(22,3,1)=21; %after #1

%block 23
pattern(23,1,1)=23; %ID
pattern(23,3,1)=24; %after #1

%block 24
pattern(24,1,1)=24; %ID
pattern(24,2,1)=23; %before #1
pattern(24,3,1)=9; %after #1

%block 25
pattern(25,1,1)=25; %ID
pattern(25,3,1)=21; %after #1

%block 26
pattern(26,1,1)=26; %ID
pattern(26,3,1)=4; %after #1

%block 27
pattern(27,1,1)=27; %ID
pattern(27,3,1)=28; %after #1

%block 28
pattern(28,1,1)=28; %ID
pattern(28,2,1)=27; %before #1
pattern(28,3,1)=4; %after #1

%block 29
pattern(29,1,1)=29; %ID
pattern(29,3,1)=12; %after #1

%block 30
pattern(30,1,1)=30; %ID
pattern(30,3,1)=1; %after #1

%block 31
pattern(31,1,1)=31; %ID
pattern(31,3,1)=7; %after #1

%block 32
pattern(32,1,1)=32; %ID
pattern(32,2,1)=1; %before #1
pattern(32,3,1)=9; %after #1

%block 33
pattern(33,1,1)=33; %ID
pattern(33,2,1)=1; %before #1
pattern(33,3,1)=9; %after #1
...
onestart=input('Will your progression START on I? "1" for yes, "0" for no.\n');
oneend=input('Will your progression END on I? "1" for yes, "0" for no.\n');
